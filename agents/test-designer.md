...
- Your output goes into the plan file verbatim — the caller (`plan-feature`) does no rewriting, cleanup, or translation — so it must already conform to the format and content restrictions in the preloaded `test-designing-guide` skill.
