...
| `BuildLocation`    | Keep the path, fix the extension: `.app` (macOS), `.exe` (Windows), no extension (Linux)                                                                                                                                                                                                                  |
...
Note: while `PlayerTestRunner.cs` exists in the project, its `TestPlayerBuildModifier` attribute applies to **every** test player build, including ones started from the Test Runner window — one more reason to delete the script right after verification.
...
2. **Player never launched** — `PlayerTestLauncher` only launches builds whose output path equals `BuildLocation`. If the project defines its own `ITestPlayerBuildModifier` that overrides `locationPathName`, the paths no longer match and nothing is launched; check the Editor log for the actual build output path and adjust `BuildLocation` (or the guard) accordingly.
...
4. **Player could not report back** — PlayerConnection uses a local network connection; an OS firewall prompt (macOS "Allow incoming connections", Windows Defender) may be blocking it. The player runs the tests but the Editor never receives results.
